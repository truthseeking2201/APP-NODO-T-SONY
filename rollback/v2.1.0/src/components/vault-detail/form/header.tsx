...
import { useGetCoinByType, useGetVaultConfig } from "@/hooks";
...
export const FormHeader = ({ vault_id }: { vault_id: string }) => {
  ...
  const lpToken = useGetCoinByType(vaultDetails?.vault_lp_token);
      ...
      <div className="flex justify-between items-center">
        <div className="text-center pl-6">
          ...
          <div className="font-mono text-2xl font-medium">
            {userHoldings ? formatShortCurrency(userHoldings) : "--"}
          </div>
        </div>
          ...
          <LabelWithTooltip
            ...
            tooltipContent={
              <div className="text-white/80 text-xs max-w-[200px] font-sans">
                <span className="font-bold">Vault Total NDLP Supply:</span> Show
                the total supply of the vault's NDLP token at the current time
              </div>
            }
