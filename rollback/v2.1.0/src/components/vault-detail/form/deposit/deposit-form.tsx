...
const DepositForm = ({ vault_id }: { vault_id: string }) => {
      ...
      vault?.tokens
        .map((token) => {
          ...
          return {
            ...
            max_deposit_amount: token?.max_deposit_amount,
          };
        })
        .sort((a, b) =>
  ...
  const lpToken = useGetCoinByType(vault.vault_lp_token);

  const methods = useForm({
    defaultValues: {
      amount: "",
      token: USDC_CONFIG.symbol,
    },
    mode: "onChange",
  });
  ...
  const collateralToken = paymentTokens.find(
    (token) =>
      token.symbol.toLowerCase() === methods.watch("token").toLowerCase()
  );
  ...
  const currentToken = useMemo(() => {
    return paymentTokens.find(
      (token) => token.symbol.toLowerCase() === paymentToken.toLowerCase()
    );
  ...
  const estimatedDepositAmount = useMemo(() => {
    return +debounceAmount > 0
      ? debounceAmount
      : new BigNumber(1).dividedBy(currentToken.decimals).toString();
  }, [debounceAmount, currentToken]);
  ...
  const {
    ...
    deposit_token: currentToken?.token_address,
  });
  ...
  const handleDepositSuccessCallback = ({
    ...
    const timeoutId = setTimeout(async () => {
      setDepositSuccessData({
        ...
        digest,
        depositLpAmount: getBalanceAmountForInput(
    ...
    <FormProvider {...methods}>
      <div className="mt-8">
        <form onSubmit={methods.handleSubmit(onSubmit)}>
          ...
          <DepositInput paymentTokens={paymentTokens} />
          {errors?.amount && (
