...
const MINIMUM_SUI_BALANCE = 0.01; // Reserve 0.01 SUI for gas fees
...
const buildTarget = (
  ...
  if (coin.coin_type === USDC_CONFIG.id) {
...
const validateDepositGasFee = async (suiClient: SuiClient, address: string) => {
  ...
  if (totalSuiBalance < MINIMUM_SUI_BALANCE) {
    throw new Error(
      `Insufficient SUI balance. You need at least ${MINIMUM_SUI_BALANCE} SUI ` +
        `(${MINIMUM_SUI_BALANCE} for gas fees). Current balance: ${totalSuiBalance.toFixed(
          6
        )} SUI`
    );
  }
...
export const useDepositVault = (vaultId: string) => {
  ...
  const deposit = async ({
    ...
    try {
      ...
      if (coin.coin_type === SUI_CONFIG.id) {
